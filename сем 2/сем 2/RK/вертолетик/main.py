...
menu_inf.add_command(label = 'Информация о программе', command = lambda: mb.showinfo('Информация о программе',\
                                                                                     "Анимация: вертолёт взлетает по синусоиде и лопасти крутятся."))
...
# В цикле вызывается функция draw_img, которая отрисовывает анимацию на pygame
while running:
    angle, degrees, sth, x_main, y_main = draw_img(okno, flag, game_clock, x_main, y_main, angle, degrees, sth)
    running = draw_img2(window, running)

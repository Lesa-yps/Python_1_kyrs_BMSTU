...
def real_table(a, b, h, table, eps, N_max):
    ...
    while (h > 0 and (a + h) <= b) or (h < 0 and (a + h) >= b):
        ...
        if True in mask and gran == 0:
            ...
            gran = 0
            # если корня на отрезке нет выведется код ошибки 10
            line = ['-', '[ {:^7.2f}; {:^7.2f} ]'.format(a, a + h), '-', '-', '-', '10']
            table.insert('', tk.END, values = line)
        # смещение отрезка
        a += h
...
def tables():
    ...
    # при ошибочных входных данных в таблицу выводится код ошибки 1
        ...
        real_table(a, b, h, table, eps, N_max)
...
# Проверка поля функции
def check():
    a = func.get()
    ind = func.index(tk.INSERT)
    if ind != 0:
        if not a[ind - 1].isdigit() and a[ind - 1] not in "()-+*/^ ,.xlnog":
            a = a[:ind - 1] + a[ind:]
            func.delete(0, tk.END)
            func.insert(0, a)    

...
def hat():
    print('-' * (ch))
    print('| фамилия студента | баллы по программированию | зачет/незачет по ангему | размер стипендии |')
    print('-' * (ch))
    ...
    while l[3] == None:
                        try:
                            ...
                            if len(str(l[3])[:str(l[3]).find('.')]) > 5 or len(str(l[3])[(str(l[3]).find('.') + 1):]) > 2:
                                    ...
                                    l[3] = float((str(l[3])[:str(l[3]).find('.')])[-5:] + '.' + (str(l[3])[(str(l[3]).find('.') + 1):])[:2])
                            if 0 > l[3]:
